...
export class BranchPayrollReportPage extends PageBase {
    ...
    syncFromHR() {
        this.env.showLoading('Đang đồng bộ dữ liệu, xin vui lòng chờ',
            this.pageProvider.commonService.connect(
                'GET', environment.appDomain + 'api/JOBS/SyncHRPayrollPerBranch',
                { Month: this.selectedCycle.Month, Year: this.selectedCycle.Year }
            ).toPromise()).then(resp => {
                this.refresh();
            }).catch(err => {
    ...
    syncToSAP() {
        ...
        this.env.showLoading('Đang đồng bộ dữ liệu, xin vui lòng chờ',
            this.pageProvider.commonService.connect(
                'GET', environment.appDomain + 'api/JOBS/SyncHRPayrollPerBranchToSAP',
                { Month: this.selectedCycle.Month, Year: this.selectedCycle.Year }
            ).toPromise()).then(resp => {
