...
export class SaleSalemanPage extends PageBase {
  ...
  readSaleSalemanReport() {
    ...
    let apiPath = {
      method: 'GET',
      url: function () {
        return ApiSetting.apiDomain('SALE/Order/SaleSalemanReport/');
      },
    };
    ...
    this.loadingController
      ...
      .then((loading) => {
        loading.present();
        this.commonService
          .connect('GET', ApiSetting.apiDomain('POS/Report/Shift/'), this.reportQuery)
          .toPromise()
          .then((resp: any) => {
  ...
  buildProductReport(resp) {
    ...
    this.ReportResult.Vietcombank = resp['PaymentAmounts']
      .filter((x) => x.SubType == 'VCB')
      .map((x) => x.TotalReceive)
      .reduce((a, b) => +a + +b, 0);
    this.ReportResult.MBBank = resp['PaymentAmounts']
      .filter((x) => x.SubType == 'MB')
      .map((x) => x.TotalReceive)
      .reduce((a, b) => +a + +b, 0);

    this.ReportResult.Cash = resp['PaymentAmounts']
      .filter((x) => x.PaymentType == 'Cash')
      .map((x) => x.TotalReceive)
      .reduce((a, b) => +a + +b, 0);
    this.ReportResult.Card = resp['PaymentAmounts']
      .filter((x) => x.PaymentType == 'Card')
      .map((x) => x.TotalReceive)
      .reduce((a, b) => +a + +b, 0);
    this.ReportResult.Transfer = resp['PaymentAmounts']
      .filter((x) => x.PaymentType == 'Transfer')
      .map((x) => x.TotalReceive)
      .reduce((a, b) => +a + +b, 0);
