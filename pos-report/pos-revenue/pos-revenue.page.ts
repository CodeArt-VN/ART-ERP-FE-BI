...
export class PosRevenuePage extends PageBase {
  ...
  loadedData(event?: any): void {
    ...
    let apiPath = {
      method: 'GET',
      url: function () {
        return ApiSetting.apiDomain('POS/Report/Revenue');
      },
    };
      ...
      if (tempIDItemGroupList.length) {
        for (let index = 0; index < tempIDItemGroupList.length; index++) {
          ...
          if (this.segmentView == 'Item') {
            tempChild = tempList.map((i) => ({
              name: i.Name,
              value: i.TotalRevenue,
              percent: ((i.TotalRevenue / this.summaryInfo.TotalRevenue) * 100).toFixed(2),
            }));
              ...
              children: [
                {
                  name: tempList[0].ItemGroup,
                  value: tempParentValue,
                  percent: ((tempParentValue / this.summaryInfo.TotalRevenue) * 100).toFixed(2),
                },
              ],
            };
          }
  ...
  buildSummaryData() {
    ...
    this.summaryInfo.AveragePerReceiptText = lib.formatMoney(
      this.summaryInfo.TotalRevenue / this.summaryInfo.TotalReceipt,
      0,
      '.',
      ',',
    );
    this.summaryInfo.AveragePerCustomerText = lib.formatMoney(
      this.summaryInfo.TotalRevenue / this.summaryInfo.TotalCustomer,
      0,
      '.',
      ',',
    );
