...
export class DynamicReportDetailPage extends PageBase {
	...
	loadItems(resp) {
		...
		if (resp.ComparitionData || resp.comparitionData || this.comparitionData.length > 0) {
			...
			if (intervalType == 'Week' || intervalType == 'Month' || intervalType == 'Quater' || intervalType == 'Year') return;
			...
			let startCompareDate: Date = this.pageProvider.calcTimeValue(this.item.DataConfig.CompareTo, false, this.item.DataConfig.TimeFrame.From);
			...
			for (let item of data) {
				...
				let compareItem = this.comparitionData.find((c) => {
					...
					} else if (intervalType == 'Week' || intervalType == 'Month' || intervalType == 'Quater' || intervalType == 'Year') {
						return false;
					}
			...
			for (let item of data) for (let m of measures) item['Percent ' + m] = item['Prev ' + m] ? ((item[m] - item['Prev ' + m]) * 100) / item['Prev ' + m] : 0;
	...
	onExportData() {
		if (this.items && this.items.length > 0) {
			...
			compareByHeader = compareByHeader?.filter(
				(c) =>
					((!this.treeConfig.isTreeList || c != this.treeConfig.treeColumn) && this.treeConfig.excludes.indexOf(c) == -1) ||
