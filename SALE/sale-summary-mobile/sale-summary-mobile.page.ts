...
import { ActionSheetController, AlertController, LoadingController, NavController, NavParams, PopoverController } from '@ionic/angular';
...
export class SaleSummaryMobilePage extends PageBase {
	...
	async presentPopover(ev: any) {
		...
		popover.onDidDismiss().then((result: any) => {
			...
			if (result.data) {
				...
				this.reportQuery._saleman = result.data.staff
					? result.data.staff
					: {
							Id: this.env.user.StaffID,
							Code: '',
	...
	readSaleProductData() {
		...
		this.pageProvider.commonService
			.connect('GET', ApiSetting.apiDomain('SALE/Order/SaleProductReport/'), this.reportQuery)
			.toPromise()
			.then((resp: any) => {
	...
	readSaleOrderData() {
		this.submitAttempt = true;
		this.pageProvider.commonService
			.connect('GET', ApiSetting.apiDomain('SALE/Order/SaleOrderReport/'), this.reportQuery)
			.toPromise()
			.then((resp: any) => {
				this.submitAttempt = false;
	...
	myHeaderFn(record, recordIndex, records) {
		if (recordIndex == 0 || record.IDSaleOrder != records[recordIndex - 1].IDSaleOrder) {
			let sum = lib.sumInArray(
				records.filter((d) => d.IDSaleOrder == record.IDSaleOrder),
